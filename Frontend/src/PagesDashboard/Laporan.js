...
export default function Laporan(props) {
    ...
    let Modelbuku = () => {
        axios.get("http://localhost:8000/menu/get", token)
            .then(data => {
    ...
    let save=()=>{
        ...
        axios.post("http://localhost:8000/transaksi/add", data, token)
            .then(data => {
                                ...
                                <div className='row'>
                                    {menu.map((item) => (
                                        <div className='col-md-4 p-2'>
                                            <img className='rounded' height={'100'} src={'http://localhost:8000/foto_menu/' + item.cover} alt={item.gambar}></img>
                                            <h3>{item.gambar}</h3>
                                            <div>${item.harga}</div>
                                            <div>
